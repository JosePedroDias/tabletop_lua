...
local function prettystr_sub(v, indentLevel, printTableRefs, cycleDetectTable)
  ...
  elseif "table" == type_v then
    ...
    -- end
    return M.private._table_tostring(v, indentLevel, printTableRefs,
                                     cycleDetectTable)
...
function M.adjust_err_msg_with_iter(err_msg, iter_msg)
  ...
  if (err_msg:find(M.FAILURE_PREFIX) == 1) or
    (err_msg:match("(" .. RE_FILE_LINE .. ")" .. M.FAILURE_PREFIX .. ".*") ~=
      nil) then
    -- substitute prefix by iteration message
...
local function mismatchFormattingPureList(table_a, table_b)
  ...
  if len_a == len_b then
    ...
    extendWithStrFmt(result, "* lists %sA (%s) and %sB (%s) have the same size",
                     refa, descrTa, refb, descrTb)
  else
    extendWithStrFmt(result,
                     "* list sizes differ: list %sA (%s) has %d items, list %sB (%s) has %d items",
                     refa, descrTa, len_a, refb, descrTb, len_b)
  end

  extendWithStrFmt(result, "* lists A and B start differing at index %d",
                   commonUntil + 1)
  if commonBackTo >= 0 then
    if deltalv > 0 then
      ...
      extendWithStrFmt(result, "* lists A and B are equal again from index %d",
                       len_a - commonBackTo)
    end
  end
  ...
  local function insertABValue(ai, bi)
    ...
    if is_equal(table_a[ai], table_b[bi]) then
      return extendWithStrFmt(result, "  = A[%d], B[%d]: %s", ai, bi,
...
local function _table_tostring(
  tbl, indentLevel, printTableRefs, cycleDetectTable)
  ...
  if mt and mt.__tostring then
    ...
    if type(result) ~= "string" then
      return string.format("<invalid tostring() result: \"%s\" >",
                           prettystr(result))
    end
    result = strsplit("\n", result)
    return M.private
             ._table_tostring_format_multiline_string(result, indentLevel)
    ...
    for k, v in sortedPairs(tbl) do
      ...
      if cycleDetectTable[v] then
        ...
        entry = entry ..
                  prettystr_sub(v, indentLevel + 1, printTableRefs,
                                cycleDetectTable)
      end
      count = count + 1
      result[count] = entry
    end
    return M.private._table_tostring_format_result(tbl, result, indentLevel,
                                                   printTableRefs)
  end
...
local function _table_tostring_format_result(
  tbl, result, indentLevel, printTableRefs)
...
local function table_findkeyof(t, element)
  ...
  if type(t) == "table" then
    for k, v in pairs(t) do if is_equal(v, element) then return k end end
  end
  return nil
end
...
function M.skipIf(cond, msg)
  ...
  if cond then error(M.SKIP_PREFIX .. msg, 2) end
end

function M.runOnlyIf(cond, msg)
  -- continue a running test if condition is met, else skip it
  if not cond then error(M.SKIP_PREFIX .. prettystr(msg), 2) end
end
...
function M.successIf(cond)
  ...
  if cond then error(M.SUCCESS_PREFIX, 2) end
end
...
function M.assertEquals(actual, expected, extra_msg_or_nil, doDeepAnalysis)
  if type(actual) == "table" and type(expected) == "table" then
    if not _is_table_equals(actual, expected) then
      failure(errorMsgEquality(actual, expected, doDeepAnalysis),
              extra_msg_or_nil, 2)
    end
...
function M.almostEquals(actual, expected, margin)
  if type(actual) ~= "number" or type(expected) ~= "number" or type(margin) ~=
    "number" then
    error_fmt(3,
  ...
  if margin < 0 then
    error("almostEquals: margin must not be negative, current value is " ..
            margin, 3)
  end
...
function M.assertNotEquals(actual, expected, extra_msg_or_nil)
  ...
  if type(actual) == "table" and type(expected) == "table" then
    if not _is_table_equals(actual, expected) then return end
  elseif actual ~= expected then
    return
  end
  fail_fmt(2, extra_msg_or_nil, "Received the not expected value: %s",
           prettystr(actual))
end
...
local function _assertErrorMsgEquals(stripFileAndLine, expectedMsg, func, ...)
  ...
  if no_error then
    failure("No error generated when calling function but expected error: " ..
              M.prettystr(expectedMsg), nil, 3)
  end
...
function M.assertEvalToFalse(value, extra_msg_or_nil)
  if value then
    failure("expected: false or nil, actual: " .. prettystr(value),
            extra_msg_or_nil, 2)
  end
end
...
function M.assertNotIsTrue(value, extra_msg_or_nil)
  if value == true then
    failure("expected: not true, actual: " .. prettystr(value),
            extra_msg_or_nil, 2)
  end
end
...
function M.assertNotIsFalse(value, extra_msg_or_nil)
  if value == false then
    failure("expected: not false, actual: " .. prettystr(value),
            extra_msg_or_nil, 2)
  end
end
...
for _, funcName in ipairs({
  ...
  typeExpected = typeExpected and typeExpected:lower() or
                   error("bad function name '" .. funcName ..
                           "' for type assertion")
...
for _, typeExpected in ipairs({
  "Number", "String", "Table", "Boolean", "Function", "Userdata", "Thread",
  "Nil"
}) do
...
for _, funcName in ipairs({
  "assertNotIsNumber", "assertNotIsString", "assertNotIsTable",
  "assertNotIsBoolean", "assertNotIsFunction", "assertNotIsUserdata",
  "assertNotIsThread"
}) do
...
function M.assertNotIs(actual, expected, extra_msg_or_nil)
  if actual == expected then
    ...
    M.PRINT_TABLE_REF_IN_ERROR_MSG = old_print_table_ref_in_error_msg
    fail_fmt(2, extra_msg_or_nil,
             "expected and actual object should be different: %s", s_expected)
  end
end
...
function M.assertNotIsInf(value, extra_msg_or_nil)
  if type(value) == "number" and math.abs(value) == math.huge then
    failure("expected: not infinity, actual: " .. prettystr(value),
            extra_msg_or_nil, 2)
  end
end
  ...
  if type(value) ~= "number" or value ~= 0 then
    ...
    if (1 / value == -math.huge) then
      ...
      if (1 / value ~= math.huge) then
        ...
        failure("expected: +0.0, actual: " .. prettystr(value),
                extra_msg_or_nil, 2)
      end
    end
  end
...
function M.assertNotTableContains(t, expected)
  ...
  if k ~= nil then
    t, expected = prettystrPairs(t, expected)
    fail_fmt(2, "Table %s DOES contain the unwanted element %s (at key %s)", t,
             expected, prettystr(k))
  end
end
...
local list_of_funcs = {
  ...
  {"assertStrMatches", "assert_str_matches"}, {"assertError", "assert_error"},
  ...
  {"assertIs", "assert_is"}, {"assertNotIs", "assert_not_is"},
  {"assertTableContains", "assert_table_contains"},
  {"assertNotTableContains", "assert_not_table_contains"},
  {"wrapFunctions", "WrapFunctions"}, {"wrapFunctions", "wrap_functions"},

  -- type assertions: assertIsXXX -> assert_is_xxx
  {"assertIsNumber", "assert_is_number"},
  {"assertIsString", "assert_is_string"}, {"assertIsTable", "assert_is_table"},
  {"assertIsBoolean", "assert_is_boolean"}, {"assertIsNil", "assert_is_nil"},
  {"assertIsTrue", "assert_is_true"}, {"assertIsFalse", "assert_is_false"},
  {"assertIsNaN", "assert_is_nan"}, {"assertIsInf", "assert_is_inf"},
  ...
  {"assertIsNumber", "assertNumber"}, {"assertIsString", "assertString"},
  {"assertIsTable", "assertTable"}, {"assertIsBoolean", "assertBoolean"},
  {"assertIsNil", "assertNil"}, {"assertIsTrue", "assertTrue"},
  {"assertIsFalse", "assertFalse"}, {"assertIsNaN", "assertNaN"},
  {"assertIsInf", "assertInf"}, {"assertIsPlusInf", "assertPlusInf"},
  {"assertIsMinusInf", "assertMinusInf"},
  {"assertIsPlusZero", "assertPlusZero"},
  {"assertIsMinusZero", "assertMinusZero"},
  {"assertIsFunction", "assertFunction"}, {"assertIsThread", "assertThread"},
  {"assertIsUserdata", "assertUserdata"},

  -- type assertions: assertIsXXX -> assert_xxx (luaunit v2 compat)
  {"assertIsNumber", "assert_number"}, {"assertIsString", "assert_string"},
  {"assertIsTable", "assert_table"}, {"assertIsBoolean", "assert_boolean"},
  {"assertIsNil", "assert_nil"}, {"assertIsTrue", "assert_true"},
  {"assertIsFalse", "assert_false"}, {"assertIsNaN", "assert_nan"},
  {"assertIsInf", "assert_inf"}, {"assertIsPlusInf", "assert_plus_inf"},
  ...
  {"assertNotIsNil", "assertNotNil"}, {"assertNotIsTrue", "assertNotTrue"},
  {"assertNotIsFalse", "assertNotFalse"}, {"assertNotIsNaN", "assertNotNaN"},
...
function TapOutput:updateStatus(node)
  ...
  if (node:isFailure() or node:isError()) and self.verbosity >
    M.VERBOSITY_DEFAULT then print(prefixString("#   ", node.stackTrace)) end
end
...
function JUnitOutput:endSuite()
  ...
  self.fd:write(string.format(
                  ...
                  self.result.duration, self.result.errorCount,
...
function M.LuaUnit.parseCmdLine(cmdLine)
  ...
  local function setArg(cmdArg, state)
    ...
    elseif state == SET_REPEAT then
      result["exeRepeat"] = tonumber(cmdArg) or
                              error("Malformed -r argument: " .. cmdArg)
      return
...
function NodeStatus:isNotSuccess()
  ...
  return (self.status == NodeStatus.FAIL or self.status == NodeStatus.ERROR or
           self.status == NodeStatus.SKIP)
end
...
function NodeStatus:statusXML()
  ...
  elseif self:isSkipped() then
    return table.concat({
      "            <skipped>", xmlEscape(self.msg), "</skipped>\n"
    })
  end
...
function M.LuaUnit.statusLine(result)
  ...
  local s = {
    string.format("Ran %d tests in %0.3f seconds", result.runCount,
                  result.duration),
...
function M.LuaUnit:startTest(testName)
  ...
  self.result.currentNode = NodeStatus.new(self.result.currentTestNumber,
                                           testName,
...
function M.LuaUnit:protectedCall(classInstance, methodInstance, prettyFuncName)
  ...
  if prettyFuncName then -- we do have the real method name
    err.trace = err.trace:gsub("in (%a+) 'methodInstance'",
                               "in %1 '" .. prettyFuncName .. "'")
  end
...
function M.LuaUnit:execOneFunction(
  className, methodName, classInstance, methodInstance)
  ...
  if type(methodInstance) ~= "function" then
    error(tostring(methodName) .. " must be a function, not " ..
            type(methodInstance))
  end
  ...
  for iter_n = 1, self.exeRepeat or 1 do
    ...
    if classInstance then
      ...
      if func then
        self:updateStatus(self:protectedCall(classInstance, func,
                                             className .. ".setUp"))
      end
    end
    ...
    -- run testMethod()
    if node:isSuccess() then
      self:updateStatus(self:protectedCall(classInstance, methodInstance,
                                           prettyFuncName))
    end
...
function M.LuaUnit.expandOneClass(result, className, classInstance)
  ...
  for methodName, methodInstance in sortedPairs(classInstance) do
    if M.LuaUnit.asFunction(methodInstance) and
      M.LuaUnit.isMethodTestName(methodName) then
  ...
  for i, v in ipairs(listOfNameAndInst) do
    ...
    if M.LuaUnit.asFunction(instance) then
      ...
      if type(instance) ~= "table" then
        error(
          "Instance must be a table or a function, not a " .. type(instance) ..
            " with value " .. prettystr(instance))
      end
...
function M.LuaUnit:runSuiteByNames(listOfName)
  ...
  for i, name in ipairs(listOfName) do
    ...
    if className then
      ...
      if type(instance) ~= "table" then
        error("Instance of " .. instanceName .. " must be a table, not " ..
                type(instance))
      end
